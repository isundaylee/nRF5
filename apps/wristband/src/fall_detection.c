...
void Madg_orientation(madgwick_t *mad, float imu_ax, float imu_ay, float imu_az, float imu_gx, float imu_gy, float imu_gz, float *roll, float *pitch, float *yaw) {
...
bool fall_detection(madgwick_t *mad, float imu_ax, float imu_ay, float imu_az, float imu_gx, float imu_gy, float imu_gz){
	float ax = (imu_ax)*ax_scale + ax_bias;
  	float ay = (imu_ay)*ay_scale + ay_bias;
  	float az = (imu_az)*az_scale + az_bias;
  	float gx = (imu_gx)*gx_scale + gx_bias;
	...
	madgwick_updateIMU(&mad, gx, gy, gz, ax, ay, az);
	float pitch = getPitch(&mad);
	float yaw = getYaw(&mad);
    ...
    // set a time out
	    ...
	    madgwick_updateIMU(&madgwick_filter, gx, gy, gz, ax, ay, az);
	    pitch = getPitch(&	mad); //y 
	    yaw = getYaw(&mad); //z
	    ...
	    float angle = acos((x_1*x_2+y_1*y_2+z_1*z_2)/(sqrt(sq(x_1) + sq(y_1) + sq(z_1))*sqrt(sq(x_2) + sq(y_2) + sq(z_2))));
...
bool main(madgwick_t *mad, float imu_ax, float imu_ay, float imu_az, float imu_gx, float imu_gy, float imu_gz) {
	// initiation
	madgwick_init(&mad);
	bool fall_detect;
	while (1){
		fall_detect = fall_detection(&mad, imu_ax, imu_ay, imu_az, imu_gx, imu_gy, imu_gz);
		return fall_detect;
	}
}